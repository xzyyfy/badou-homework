...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x = self.dropout(x)  # put shape:(batch_size, sen_len, hidden_size)
        x = self.bn1(x.transpose(1, 2)).transpose(1, 2)  # put shape:(batch_size, sen_len, hidden_size)
        x = self.classify(x)  # output shape:(batch_size, sen_len, 2)
        x = self.relu(x)  # output shape:(batch_size, sen_len, 2)
        x = self.dropout(x)  # output shape:(batch_size, sen_len, 2)
        y_pred = self.bn2(x.transpose(1, 2)).transpose(1, 2)  # output shape:(batch_size, sen_len, 2)
        if y is not None:
            # view(-1,2): (batch_size, sen_len, 2) ->  (batch_size * sen_len, 2)
            return self.loss_func(y_pred.contiguous().view(-1, 2), y.view(-1))
...
def main():
    ...
    for epoch in range(epoch_num):
        ...
        print("=========\n第%d轮平均loss:%f" % (epoch + 1, np.mean(np.array(watch_loss))))
    # 保存模型
...
if __name__ == "__main__":
    ...
    #                  "经过两个交易日的强势调整后",
    #                  "昨日上海天然橡胶期货价格再度大幅上扬"]
    # predict("model_lstm.pth", "chars.txt", input_strings)
