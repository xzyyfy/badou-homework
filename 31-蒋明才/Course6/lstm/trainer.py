...
def main():
    ...
    model = base_model.BiLSTMAttention(config.input_dim, config.hidden_dim, config.out_dim, config.vocab_size)
    LSMTTrain(model, train_loader, dev_loader, config.lr, config.epoch, config.model_save_path, writer, config.wait).train()
