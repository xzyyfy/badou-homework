...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, sentence1, sentence2=None, sentence3=None, target=None):
        ...
        if sentence3 is not None:  # 当sentence3不为空时，使用cosine_triplet_loss
            vector1 = self.sentence_encoder(sentence1)  # vec:(batch_size, hidden_size)
            vector2 = self.sentence_encoder(sentence2)  # 文本表示型的孪生网络
            vector3 = self.sentence_encoder(sentence3)
            # 如果有标签，则计算loss，预测时返回的是cosine_distance
            if target is not None:
                return self.cosine_triplet_loss(vector1, vector2, vector3, None)
            # 如果无标签，计算余弦距离
            else:
...
if __name__ == "__main__":
    ...
    s3 = torch.LongTensor([[1, 0, 3, 4], [3, 0, 3, 4]])
    l = torch.LongTensor([[1], [0]])
    y = model(s1, s2, None, l)
    print(y)
