...
class Corrector:
    ...
    def correction(self, string):
        #todo  自己尝试实现
        to_replace = {}
        before = self.language_model.predict(string)
        for i, word in enumerate(string):
            similar_pronounce = self.sub_dict.get(word, [])
            if not similar_pronounce:
                continue
            for each in similar_pronounce:
                after = self.language_model.predict(string[:i]+each+string[i+1:])
                if after - before >= self.threshold:
                    to_replace[i] = each
                    break
        for i, each in to_replace.items():
