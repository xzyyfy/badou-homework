...
class TorchModel(nn.Module):
    def __init__(self, input_dim, hidden_size1, hidden_size2, vocab):
        super(TorchModel, self).__init__()
        self.embedding = nn.Embedding(len(vocab) + 1, input_dim) #shape=(vocab_size, dim)
        self.lstm_layer1 = nn.LSTM(input_dim, hidden_size1, batch_first=True)
        self.lstm_layer2 = nn.LSTM(hidden_size1, hidden_size2, batch_first=True)
        self.classify = nn.Linear(hidden_size2, 2)
        self.loss_func = nn.CrossEntropyLoss(ignore_index=-100)

    #当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.embedding(x)  #input shape: (batch_size, sen_len), output shape:(batch_size, sen_len, input_dim)
        x, _ = self.lstm_layer1(x)  #output shape:(batch_size, sen_len, hidden_size)
        x, _ = self.lstm_layer2(x)  #output shape:(batch_size, sen_len, hidden_size)
...
def main():
    ...
    char_dim = 64         #每个字的维度
    hidden_size1 = 128  # 隐含层维度
    hidden_size2 = 64  # 隐含层维度
    num_rnn_layers = 3    #rnn层数
    max_length = 32       #样本最大长度
    ...
    model = TorchModel(char_dim, hidden_size1, hidden_size2, vocab)   #建立模型
...
if __name__ == "__main__":
    ...
    if 0 == ans:
        ...
        input_strings = ["北京的万里长城可不是一般的长城，而是有着举世瞩目的意义",
                        "我特别喜欢外出去欣赏尤其是像桂林山水那样美不胜收的自然景观",
                        "南京市长江大桥位于会穿越经过南京市的长江上面",
                        "昨日上海天然橡胶期货价格再度大幅上扬造成严重的交通事故"]
