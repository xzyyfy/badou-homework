...
class TorchModel(nn.Module):
    def __init__(self, input_dim, hidden_size, num_rnn_layers, vocab):
        ...
        self.rnn_layer = nn.RNN(input_size=input_dim,
                            ...
                            batch_first=True,
                            num_layers=num_rnn_layers, # RNN层数
                            dropout=0.1) # 一定程度避免过拟合
        self.classify = nn.Linear(hidden_size, 2) # 映射到二维，因为是二分类任务
        self.loss_func = nn.CrossEntropyLoss(ignore_index=-100) # 交叉熵为损失函数

    #当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.embedding(x)  #input_size：(batch_size, sentence_length), output shape:(batch_size, sen_len, input_dim)
        x, _ = self.rnn_layer(x)  #output shape:(batch_size, sen_len, hidden_size)
        # _隐单元向量，最后一个表达式的值，同时，当接收函数的多个返回值时，其中某一个是我们不需要的，为了这个不需要的返回值，起一个变量名实在麻烦，这时候，你就可以用下划线来处理这种情况，这个返回值不会储存
        y_pred = self.classify(x)   #output shape:(batch_size, sen_len)
        if y is not None:
